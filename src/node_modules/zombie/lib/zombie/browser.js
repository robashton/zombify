...
var BROWSER_OPTIONS, Browser, Cache, Cookies, Credentials, EventEmitter, EventLoop, FS, GLOBAL_OPTIONS, HTML, HTML5, Interact, JSDOM, MOUSE_EVENT_NAMES, Mime, PACKAGE, Path, Q, Resources, Storages, URL, VERSION, XHR, createTabs, deprecated, format,
...
BROWSER_OPTIONS = ["debug", "headers", "htmlParser", "loadCSS", "maxWait", "proxy", "referer", "runScripts", "silent", "site", "userAgent", "waitFor", "maxRedirects"];

GLOBAL_OPTIONS = ["debug", "headers", "htmlParser", "loadCSS", "maxWait", "proxy", "runScripts", "silent", "site", "userAgent", "waitFor", "maxRedirects"];
...
Browser = (function(_super) {
  ...
  Browser.prototype.wait = function(duration, callback) {
    var completion, deferred, last, _ref, _ref1,
      _this = this;
    if (!this.window) {
    ...
    if (typeof duration === "function") {
      _ref1 = [duration, null], completion = _ref1[0], duration = _ref1[1];
    }
    duration || (duration = this.maxWait);
    deferred = Q.defer();
    if (callback) {
      deferred.promise.then(callback, callback);
    }
    last = this.errors[this.errors.length - 1];
    this._eventLoop.wait(duration, completion, function(error, timedOut) {
      var newest;
